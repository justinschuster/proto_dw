...
1. Keep this skill at `./.opencode/skills/code-change-verification` so it loads automatically for the repository.
2. macOS/Linux: `bash .opencode/skills/code-change-verification/scripts/run.sh`.
3. The scripts run `make format` first, then run `make lint`, `make typecheck`, and `make tests` in parallel with fail-fast semantics.
